...
export const Route = createFileRoute("/_admin/admin/valuations")({ component: AdminValuationsPage });

// Re-export the full valuations page under the admin shell.
// We duplicate the component here to avoid cross-route imports; the logic is identical.
...
function AdminValuationsPage() {
  ...
  function copyValuation(v: Valuation) {
    setEditing({ ...v, id: "__copy__", valuation_date: new Date().toISOString().slice(0, 10), purchase_price: null, purchase_date: null, sale_price: null, sale_date: null, images: [] });
    setDetail(null);
    setModalOpen(true);
  }
  ...
  return (
    <div className="p-8 space-y-6 w-full">
      <header className="flex items-end justify-between gap-4 flex-wrap">
        ...
        <button onClick={openCreate} className="inline-flex items-center gap-2 px-4 py-2 rounded-md bg-primary text-primary-foreground text-sm hover:opacity-90">
      ...
      <div className="space-y-3">
        <div className="flex flex-wrap gap-3 items-center">
          <div className="relative flex-1 min-w-[240px] max-w-md">
            ...
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Sök märke, modell, typ, utrustning…" className="w-full pl-9 pr-3 py-2.5 rounded-full border bg-card text-sm placeholder:text-muted-foreground" />
          </div>
          <button onClick={() => setFiltersOpen((o) => !o)} className={`inline-flex items-center gap-2 px-3.5 py-2.5 rounded-full border text-sm transition-colors ${hasFilters ? "bg-primary text-primary-foreground border-primary" : "bg-card hover:bg-muted"}`}>
          ...
          {hasFilters && <button onClick={() => { setFBrand(""); setFType(""); setFAxle(""); setFBuilder(""); setFYearMin(""); setFYearMax(""); setFMileMin(""); setFMileMax(""); setFPriceMin(""); setFPriceMax(""); }} className="text-xs text-muted-foreground hover:text-foreground underline">Rensa filter</button>}
        </div>
          ...
          <div className="rounded-xl border bg-card p-4 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            <div className="space-y-1">
              ...
              <select value={fBrand} onChange={(e) => setFBrand(e.target.value)} className="w-full text-sm px-2 py-1.5 rounded-md border bg-background">
                <option value="">Alla</option>{brands.map((b) => <option key={b} value={b}>{b}</option>)}
              </select>
            </div>
            <div className="space-y-1">
              ...
              <select value={fType} onChange={(e) => setFType(e.target.value)} className="w-full text-sm px-2 py-1.5 rounded-md border bg-background">
                <option value="">Alla</option>{types.map((t) => <option key={t} value={t}>{t}</option>)}
              </select>
            </div>
            <div className="space-y-1">
              ...
              <select value={fAxle} onChange={(e) => setFAxle(e.target.value)} className="w-full text-sm px-2 py-1.5 rounded-md border bg-background">
                <option value="">Alla</option>{axles.map((a) => <option key={a} value={a}>{a}</option>)}
              </select>
            </div>
            <div className="space-y-1">
              ...
              <select value={fBuilder} onChange={(e) => setFBuilder(e.target.value)} className="w-full text-sm px-2 py-1.5 rounded-md border bg-background">
                <option value="">Alla</option>{builders.map((b) => <option key={b} value={b}>{b}</option>)}
              </select>
            </div>
            <div className="space-y-1">
      ...
      {loading ? (
        ...
        <div className="grid gap-3">
          {filtered.map((v) => (
            <ValuationCard key={v.id} v={v} onClick={() => setDetail(v)} onEdit={() => openEdit(v)} onCopy={() => copyValuation(v)} onDelete={() => deleteValuation(v)} />
          ))}
        </div>
      )}

      {detail && (
        <ValuationDetail v={detail} onClose={() => setDetail(null)} onEdit={() => openEdit(detail)} onCopy={() => copyValuation(detail)} onDelete={() => deleteValuation(detail)}
          onUpdated={(updated) => { setValuations((prev) => prev.map((x) => x.id === updated.id ? updated : x)); setDetail(updated); }} />
      )}

      {modalOpen && (
        <ValuationModal initial={editing} userId={user?.id ?? ""} onClose={() => setModalOpen(false)}
          onSaved={(saved) => { setValuations((prev) => { const exists = prev.find((x) => x.id === saved.id); return exists ? prev.map((x) => x.id === saved.id ? saved : x) : [saved, ...prev]; }); setModalOpen(false); }} />
      )}
    </div>
  );
}

function ValuationCard({ v, onClick, onEdit, onCopy, onDelete }: { v: Valuation; onClick: () => void; onEdit: () => void; onCopy: () => void; onDelete: () => void }) {
  const thumb = v.images[0] ? imageUrl(v.images[0]) : null;
  return (
    <div className="flex gap-4 bg-card border rounded-xl p-4 hover:border-primary/40 transition-colors cursor-pointer" onClick={onClick}>
      <div className="shrink-0 size-20 rounded-lg overflow-hidden bg-muted flex items-center justify-center">
        {thumb ? <img src={thumb} alt="" className="w-full h-full object-cover" /> : <ImageIcon className="size-6 text-muted-foreground/40" />}
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            <div className="font-display text-lg truncate">{[v.brand, v.model].filter(Boolean).join(" ") || "Okänt fordon"}</div>
          ...
          <div className="text-right shrink-0">
            {v.valuation_price != null && <div className="font-semibold text-sm">{fmt(v.valuation_price)} kr</div>}
            {v.valuation_date && <div className="text-xs text-muted-foreground">{new Date(v.valuation_date).toLocaleDateString("sv-SE")}</div>}
          </div>
        </div>
        {(v.purchase_price != null || v.sale_price != null) && (
          <div className="flex gap-3 mt-1.5 text-xs">
            {v.purchase_price != null && <span className="text-info">Inköp: {fmt(v.purchase_price)} kr</span>}
            {v.sale_price != null && <span className="text-success">Försäljning: {fmt(v.sale_price)} kr</span>}
          </div>
        )}
      </div>
...
function ValuationDetail({ v, onClose, onEdit, onCopy, onDelete, onUpdated }: { v: Valuation; onClose: () => void; onEdit: () => void; onCopy: () => void; onDelete: () => void; onUpdated: (v: Valuation) => void }) {
  ...
  async function savePurchase() {
    ...
    const { data, error } = await supabase.from("valuations" as any).update({ purchase_price: purchasePrice ? parseInt(purchasePrice) : null, purchase_date: purchaseDate || null }).eq("id", v.id).select().single();
  ...
  async function saveSale() {
    setSavingSale(true);
    const { data, error } = await supabase.from("valuations" as any).update({ sale_price: salePrice ? parseInt(salePrice) : null, sale_date: saleDate || null }).eq("id", v.id).select().single();
    setSavingSale(false);
    if (error) return toast.error(error.message);
    onUpdated(data as unknown as Valuation);
        ...
        <div className="p-6 space-y-6">
          ...
          <section>
            ...
            <dl className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
              <F label="Märke" value={v.brand} /><F label="Modell" value={v.model} />
              <F label="Fordonstyp/påbyggnad" value={v.vehicle_type} /><F label="Axelkonfiguration" value={v.axle_config} />
              <F label="Årsmodell" value={v.year?.toString()} /><F label="Miltal" value={v.mileage != null ? `${fmt(v.mileage)} mil` : null} />
              <F label="Påbyggare" value={v.body_builder} /><F label="Kapacitet" value={v.capacity} />
              {v.equipment && <div className="col-span-2"><F label="Utrustning" value={v.equipment} /></div>}
            </dl>
          </section>
          <section>
            <h3 className="text-[11px] font-semibold uppercase tracking-widest text-muted-foreground mb-3">Värdering</h3>
            <dl className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
              <F label="Värderat inköpspris" value={v.valuation_price != null ? `${fmt(v.valuation_price)} kr` : null} />
              <F label="Datum för värdering" value={v.valuation_date ? new Date(v.valuation_date).toLocaleDateString("sv-SE") : null} />
            </dl>
          </section>
          <section>
            <h3 className="text-[11px] font-semibold uppercase tracking-widest text-muted-foreground mb-3">Faktiskt inköp</h3>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1"><label className="text-xs text-muted-foreground">Inköpspris (kr)</label><input type="number" value={purchasePrice} onChange={(e) => setPurchasePrice(e.target.value)} placeholder="—" className="w-full px-3 py-2 rounded-md border bg-background text-sm" /></div>
              <div className="space-y-1"><label className="text-xs text-muted-foreground">Inköpsdatum</label><input type="date" value={purchaseDate} onChange={(e) => setPurchaseDate(e.target.value)} className="w-full px-3 py-2 rounded-md border bg-background text-sm" /></div>
            </div>
            ...
            {valuationDiff != null && <p className={`mt-2 text-xs ${valuationDiff > 0 ? "text-destructive" : "text-success"}`}>{valuationDiff > 0 ? `Betalade ${fmt(valuationDiff)} kr mer än värderat` : `Betalade ${fmt(Math.abs(valuationDiff))} kr mindre än värderat`}</p>}
          </section>
          <section>
            <h3 className="text-[11px] font-semibold uppercase tracking-widest text-muted-foreground mb-3">Försäljning</h3>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1"><label className="text-xs text-muted-foreground">Försäljningspris (kr)</label><input type="number" value={salePrice} onChange={(e) => setSalePrice(e.target.value)} placeholder="—" className="w-full px-3 py-2 rounded-md border bg-background text-sm" /></div>
              <div className="space-y-1"><label className="text-xs text-muted-foreground">Försäljningsdatum</label><input type="date" value={saleDate} onChange={(e) => setSaleDate(e.target.value)} className="w-full px-3 py-2 rounded-md border bg-background text-sm" /></div>
            </div>
            ...
            {margin != null && <p className={`mt-2 text-xs ${margin >= 0 ? "text-success" : "text-destructive"}`}>{margin >= 0 ? `Marginal: +${fmt(margin)} kr` : `Förlust: ${fmt(margin)} kr`}</p>}
          </section>
          {v.notes && <section><h3 className="text-[11px] font-semibold uppercase tracking-widest text-muted-foreground mb-3">Anteckningar</h3><p className="text-sm whitespace-pre-wrap leading-relaxed">{v.notes}</p></section>}
        </div>
      </div>
    </div>
  );
...
function ValuationModal({ initial, userId, onClose, onSaved }: { initial: Valuation | null; userId: string; onClose: () => void; onSaved: (v: Valuation) => void }) {
  const isCopy = initial?.id === "__copy__";
  const isEdit = !!initial && !isCopy;

  const [form, setForm] = useState<FormData>(() => {
    if (!initial) return { ...EMPTY, valuation_date: new Date().toISOString().slice(0, 10) };
    return {
      ...
      body_builder: initial.body_builder, capacity: initial.capacity, equipment: initial.equipment,
      valuation_price: initial.valuation_price, valuation_date: initial.valuation_date ?? new Date().toISOString().slice(0, 10),
      purchase_price: isCopy ? null : initial.purchase_price, purchase_date: isCopy ? "" : (initial.purchase_date ?? ""),
      sale_price: isCopy ? null : initial.sale_price, sale_date: isCopy ? "" : (initial.sale_date ?? ""),
      notes: initial.notes, images: isCopy ? [] : initial.images,
    };
  });
  ...
  async function save() {
    setSaving(true);
    const payload = {
      brand: form.brand || null, model: form.model || null, vehicle_type: form.vehicle_type || null,
      axle_config: form.axle_config || null, year: form.year, mileage: form.mileage,
      body_builder: form.body_builder || null, capacity: form.capacity || null, equipment: form.equipment || null,
      valuation_price: form.valuation_price, valuation_date: form.valuation_date || null,
    ...
    onSaved(data as unknown as Valuation);
  }
      ...
      <div className="bg-background rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-6 space-y-6">
          <section className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <F2 label="Märke" value={form.brand ?? ""} onChange={(v) => set("brand", v)} placeholder="Volvo" />
              <F2 label="Modell" value={form.model ?? ""} onChange={(v) => set("model", v)} placeholder="FH540" />
              <F2 label="Fordonstyp/påbyggnad" value={form.vehicle_type ?? ""} onChange={(v) => set("vehicle_type", v)} placeholder="Lastväxlare" />
              <F2 label="Axelkonfiguration" value={form.axle_config ?? ""} onChange={(v) => set("axle_config", v)} placeholder="8×4" />
              <div className="space-y-1"><label className="text-xs text-muted-foreground">Årsmodell</label><input type="number" value={form.year ?? ""} onChange={(e) => set("year", e.target.value ? parseInt(e.target.value) : null)} placeholder="2021" className="w-full px-3 py-2 rounded-md border bg-background text-sm" /></div>
              <div className="space-y-1"><label className="text-xs text-muted-foreground">Miltal (mil)</label><input type="number" value={form.mileage ?? ""} onChange={(e) => set("mileage", e.target.value ? parseInt(e.target.value) : null)} placeholder="40 000" className="w-full px-3 py-2 rounded-md border bg-background text-sm" /></div>
              <F2 label="Påbyggare" value={form.body_builder ?? ""} onChange={(v) => set("body_builder", v)} placeholder="JOAB" />
              <F2 label="Kapacitet" value={form.capacity ?? ""} onChange={(v) => set("capacity", v)} placeholder="24 ton" />
            </div>
            <div className="space-y-1"><label className="text-xs text-muted-foreground">Utrustning och övrig information</label><input value={form.equipment ?? ""} onChange={(e) => set("equipment", e.target.value)} placeholder="T.ex. backamera, dragkrok, AC…" className="w-full px-3 py-2 rounded-md border bg-background text-sm" /></div>
          </section>
          <section className="space-y-3">
            <h3 className="text-[11px] font-semibold uppercase tracking-widest text-muted-foreground">Värdering</h3>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1"><label className="text-xs text-muted-foreground">Värderat inköpspris (kr)</label><input type="number" value={form.valuation_price ?? ""} onChange={(e) => set("valuation_price", e.target.value ? parseInt(e.target.value) : null)} placeholder="0" className="w-full px-3 py-2 rounded-md border bg-background text-sm" /></div>
              <div className="space-y-1"><label className="text-xs text-muted-foreground">Datum för värdering</label><input type="date" value={form.valuation_date ?? ""} onChange={(e) => set("valuation_date", e.target.value)} className="w-full px-3 py-2 rounded-md border bg-background text-sm" /></div>
            </div>
          </section>
          <section className="space-y-1">
            <h3 className="text-[11px] font-semibold uppercase tracking-widest text-muted-foreground mb-2">Anteckningar</h3>
            <textarea value={form.notes ?? ""} onChange={(e) => set("notes", e.target.value)} rows={5} placeholder="Skick, däck, servicehistorik, skador, extrautrustning, resonemang kring värderingen…" className="w-full px-3 py-2 rounded-md border bg-background text-sm resize-none" />
          </section>
            ...
            <div className="flex flex-wrap gap-2">
              {form.images.map((path) => (
                <div key={path} className="relative size-20 rounded-lg overflow-hidden border group">
                  ...
                  <button onClick={() => removeImage(path)} className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity"><X className="size-4 text-white" /></button>
                </div>
              ))}
              <button onClick={() => fileRef.current?.click()} disabled={uploading} className="size-20 rounded-lg border-2 border-dashed flex flex-col items-center justify-center gap-1 hover:bg-muted transition-colors text-muted-foreground disabled:opacity-50">
...
function F2({ label, value, onChange, placeholder }: { label: string; value: string; onChange: (v: string) => void; placeholder?: string }) {
  return <div className="space-y-1"><label className="text-xs text-muted-foreground">{label}</label><input value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} className="w-full px-3 py-2 rounded-md border bg-background text-sm" /></div>;
}
